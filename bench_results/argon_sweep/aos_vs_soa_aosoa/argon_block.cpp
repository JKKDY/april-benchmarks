...
std::vector<Experiment> plan_benchmarks(const double target_runtime_sec = 10.0, const double expected_mups_per_core = 1) {
    ...
    const std::vector<size_t> threads = {1,2,4,8,16,24, 32};
...
void run_argon_bench_suiteAoS(const std::vector<Experiment>& experiments) {
    ...
    std::cout << "================================================\n";
    std::cout << " AOS SUITE STARTING: " << suite_dir.string() << "\n";
    ...
    for (size_t i = 0; i < experiments.size(); ++i) {
        ...
        auto container = LinkedCells<Layout::AoS>().with_absolute_skin(0.3);
...
void run_argon_bench_suiteSoA(const std::vector<Experiment>& experiments) {
    namespace fs = std::filesystem;
...
void run_argon_bench_suiteAoSoA(const std::vector<Experiment>& experiments) {
    namespace fs = std::filesystem;
    ...
    std::cout << "packed size: " << std::to_string(sizeof(april::packed::size())) << std::endl;

    std::cout << "================================================\n";
    std::cout << " AOSOA SUITE STARTING: " << suite_dir.string() << "\n";
    std::cout << " RUNNING " << experiments.size() << " EXPERIMENTS\n";
    std::cout << "================================================\n";
...
int main() {
    ...
    run_argon_bench_suiteAoS(plan);
    run_argon_bench_suiteSoA(plan);
    run_argon_bench_suiteAoSoA(plan);
