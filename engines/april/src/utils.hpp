...
template <typename RunSim>
void run_benchmark(const int argc, char* argv[], int n_args, RunSim run_sim) {
    if (argc < n_args + 4) {
        std::cerr << "Usage: [env_args...] dt t steps [bx by bz] [sched] [layout] [exec] [ordering] [tag]" << std::endl;
        return;
    }
    ...
    if (argc > n_args + 7) {
        ...
        if (args.schedule == "C01")           schedule = C01_schedule;
            ...
            std::cerr << "Unknown Schedule: " << args.schedule << std::endl;
            return;
        }
    }
    ...
    args.ordering = (argc > n_args + 10) ? argv[n_args + 10] : "hilbert";
    args.tag      = (argc > n_args + 11) ? argv[n_args + 11] : "default";

    if (args.ordering != "hilbert" &&
        args.ordering != "morton" &&
        args.ordering != "none") {
        std::cerr << "Unknown Ordering: " << args.ordering << std::endl;
        return;
    }
    ...
    if (args.layout == "SoA") {
        dispatch_executor.template operator()<Layout::SoA>();
    } else if (args.layout == "AoS") {
        dispatch_executor.template operator()<Layout::AoS>();
    } else if (args.layout == "AoSoA") {
        dispatch_executor.template operator()<Layout::AoSoA<>>();
    } else {
        std::cerr << "Unknown Layout: " << args.layout << std::endl;
    }
}
